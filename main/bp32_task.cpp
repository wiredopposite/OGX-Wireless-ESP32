...
void my_platform_on_controller_data(uni_hid_device_t* d, uni_controller_t* ctl) 
{
    ...
    switch (ctl->klass) 
    {
        case UNI_CONTROLLER_CLASS_GAMEPAD:
            bp_gamepad = &ctl->gamepad;

            if (bp_gamepad->dpad & DPAD_DOWN ) local_out_packet.buttons |= bp32_gamepad[idx].profile.down; 
            if (bp_gamepad->dpad & DPAD_LEFT ) local_out_packet.buttons |= bp32_gamepad[idx].profile.left; 
            if (bp_gamepad->dpad & DPAD_RIGHT) local_out_packet.buttons |= bp32_gamepad[idx].profile.right; 
            if (bp_gamepad->dpad & DPAD_UP   ) local_out_packet.buttons |= bp32_gamepad[idx].profile.up; 

            if (bp_gamepad->buttons & BUTTON_A) local_out_packet.buttons |= bp32_gamepad[idx].profile.a; 
            if (bp_gamepad->buttons & BUTTON_B) local_out_packet.buttons |= bp32_gamepad[idx].profile.b; 
            if (bp_gamepad->buttons & BUTTON_X) local_out_packet.buttons |= bp32_gamepad[idx].profile.x; 
            if (bp_gamepad->buttons & BUTTON_Y) local_out_packet.buttons |= bp32_gamepad[idx].profile.y; 

            if (bp_gamepad->buttons & BUTTON_THUMB_L)    local_out_packet.buttons |= bp32_gamepad[idx].profile.l3;
            if (bp_gamepad->buttons & BUTTON_THUMB_R)    local_out_packet.buttons |= bp32_gamepad[idx].profile.r3;
            if (bp_gamepad->buttons & BUTTON_SHOULDER_L) local_out_packet.buttons |= bp32_gamepad[idx].profile.lb;
            if (bp_gamepad->buttons & BUTTON_SHOULDER_R) local_out_packet.buttons |= bp32_gamepad[idx].profile.rb;

            if (bp_gamepad->misc_buttons & MISC_BUTTON_BACK)    local_out_packet.buttons |= bp32_gamepad[idx].profile.back;
            if (bp_gamepad->misc_buttons & MISC_BUTTON_START)   local_out_packet.buttons |= bp32_gamepad[idx].profile.start;
            if (bp_gamepad->misc_buttons & MISC_BUTTON_SYSTEM)  local_out_packet.buttons |= bp32_gamepad[idx].profile.sys;
            if (bp_gamepad->misc_buttons & MISC_BUTTON_CAPTURE) local_out_packet.buttons |= bp32_gamepad[idx].profile.misc;

            local_out_packet.lt = scale_bp_trigger(bp_gamepad->brake,    bp32_gamepad[idx].profile.deadzone.lt);
            local_out_packet.rt = scale_bp_trigger(bp_gamepad->throttle, bp32_gamepad[idx].profile.deadzone.rt);

            if (local_out_packet.lt < 1 && bp_gamepad->buttons & BUTTON_TRIGGER_L) local_out_packet.lt = UINT8_MAX;
            if (local_out_packet.rt < 1 && bp_gamepad->buttons & BUTTON_TRIGGER_R) local_out_packet.rt = UINT8_MAX;

            local_out_packet.ly = scale_bp_axis(bp_gamepad->axis_y,  bp32_gamepad[idx].profile.deadzone.ly, !bp32_gamepad[idx].profile.ly_invert);
            local_out_packet.lx = scale_bp_axis(bp_gamepad->axis_x,  bp32_gamepad[idx].profile.deadzone.ly, false);
            local_out_packet.ry = scale_bp_axis(bp_gamepad->axis_ry, bp32_gamepad[idx].profile.deadzone.ry, !bp32_gamepad[idx].profile.ry_invert);
            local_out_packet.rx = scale_bp_axis(bp_gamepad->axis_rx, bp32_gamepad[idx].profile.deadzone.ry, false);
...
void process_in_packet(int idx, i2cInPacket in_packet)
{
    ...
    static int rumble_length_ms = 150;
